...
public:
  enum class StopMode : uint8_t {
    Coast = 0,   // both PWM = 0
    Brake        // optional; only if your driver supports it (see setStopMode)
  };

  Motors();

  // If your driver has a single enable pin for both channels, pass it here.
  // If you don't have an enable pin, pass enablePin = 255 (default).
  ...
  // Signed command in range [-255, +255].
  ...
  // Convenience for normalized [-1.0, +1.0]
  ...
  // Immediately stop both motors using configured StopMode
  void stop();

  // Optional: per-motor command limiting (e.g., cap at 200)
